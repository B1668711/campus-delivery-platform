-- 添加缺失的字段到delivery_orders表

-- 添加description字段到delivery_orders表（如果不存在）
DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.columns 
        WHERE table_name = 'delivery_orders' AND column_name = 'description'
    ) THEN
        ALTER TABLE delivery_orders ADD COLUMN description TEXT;
    END IF;
END $$;

-- 添加title字段到delivery_orders表（如果不存在）
DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.columns 
...
-- 添加notes字段到delivery_orders表（如果不存在）
DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.columns 
        WHERE table_name = 'delivery_orders' AND column_name = 'notes'
    ) THEN
        ALTER TABLE delivery_orders ADD COLUMN notes TEXT;
    END IF;
END $$;

-- 更新update_order_info函数，确保正确处理所有字段
