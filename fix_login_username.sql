...
BEGIN
    ...
    -- 用户名密码登录
    SELECT u.id, u.device_id, u.name, u.username, u.password_hash INTO user_record
    FROM users u
    ...
    -- 返回用户信息，确保返回的用户名是输入的用户名
    RETURN QUERY SELECT 
        user_record.id::TEXT, 
        user_record.device_id, 
        user_record.name,
        username_in,  -- 使用输入的用户名，而不是数据库中的用户名
        TRUE, 
        '登录成功';
    RETURN;
END;
